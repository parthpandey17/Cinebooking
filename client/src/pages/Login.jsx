...
const Login = ({ admin = false }) => {
	...
	if (admin && auth.role === 'user') return <Navigate to="/" replace />
	if (admin && auth.role === 'admin') return <Navigate to="/admin" replace />

	const onSubmit = async (data) => {
		SetLoggingIn(true)
		try {
			const response = await axios.post(admin ? '/auth/admin/login' : '/auth/login', data)
			...
			<div className="w-full max-w-md space-y-8 rounded-2xl bg-white p-4 shadow-xl">
				...
				<form className="mt-8 space-y-4" onSubmit={handleSubmit(onSubmit)}>
					...
					<div>
						...
						>
							{isLoggingIn ? 'Processing...' : admin ? 'Admin Login' : 'Login'}
						</button>
					</div>
					{!admin && <p className="text-right">
