...
const ShowtimeDetails = ({ showDeleteBtn, showtime, fetchShowtime }) => {
	...
	const availability = getAvailabilityInfo({
		...
		seatPlan: showtime?.theater?.seatPlan,
			...
			<div className="flex flex-col md:flex-row">
				<div className="flex grow flex-col gap-4 bg-gradient-to-br from-indigo-100 to-white py-2 drop-shadow-lg sm:py-4">
					<div className="flex items-center">
						<img
							src={posterSrc(showtime?.movie?.img, showtime?.movie?.name)}
							alt={showtime?.movie?.name || 'Movie poster'}
							className="w-32 px-4 drop-shadow-md"
							onError={(e) => {
								e.currentTarget.onerror = null
								e.currentTarget.src = posterFallback(showtime?.movie?.name)
							}}
						/>
						<div className="flex flex-col">
							...
							<span className={`mt-2 w-fit rounded-full px-3 py-1 text-sm font-bold ${availability.className}`}>
								{availability.label} ({availability.available}/{availability.capacity})
							</span>
						</div>
					</div>
				</div>
